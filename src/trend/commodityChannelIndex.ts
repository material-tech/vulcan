...
export const cci = createSignal(
  (data: RequiredProperties<KlineData, 'h' | 'l' | 'c'>[], { period }: Required<CommodityChannelIndexOptions>) => {
    ...
    return movingAction(
      ...
      (window): Dnum => {
        ...
        const sma = divide(sum, n, 18)

        // Mean Deviation = SUM(|TP_i - SMA|) / period
        let devSum: Dnum = from(0, 18)
        for (const tp of window) {
          const diff = subtract(tp, sma)
        ...
        const numerator = subtract(currentTP, sma)
